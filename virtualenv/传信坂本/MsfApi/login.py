...
#res req
    ...
    options ={ 
        ...
        "module":{
                ...
                "execute":[ "module.execute", "<token>", "ModuleType","ModuleName",{}]
        ...
        "session":{
                ...
                "write":[ "session.shell_write", "<token>", "SessionID", "data\n" ],
    ...
    def shell_read(self,S_ID):
        S_ID=str(S_ID)
        self.options['shell']['read'][1]=self.token
        self.options['shell']['read'][2]=S_ID
        self.options['shell']['read'][3]=0
        res=self.request(option=self.options['shell']['read'])
        return res
    
    def shell_write(self,S_ID,write):
        write=write+"\n"
        self.options['shell']['write'][1]=self.token
        self.options['shell']['write'][2]=S_ID
        self.options['shell']['write'][3]=write
        res=self.request(option=self.options['shell']['write'])
        return res
    ...
    def job_list(self):
        self.options['job']['list'][1]=self.token
        res=self.request(option=self.options['job']['list'])
        return res
        #返回一个字典
    ...
    def meterpreter_write(self,S_ID,data):
        '''
        S_ID 为写入的id号
        '''
        data=data+"\n"
        self.options['meterpreter']['write'][1]=self.token
        self.options['meterpreter']['write'][2]=S_ID
        self.options['meterpreter']['write'][3]=data
        res=self.request(option=self.options['meterpreter']['write'])
        return res

    def meterpreter_read(self,S_ID):
        self.options['meterpreter']['read'][1]=self.token
        self.options['meterpreter']['read'][2]=S_ID

        res=self.request(option=self.options['meterpreter']['read'])
        return res
